...
namespace Jobs.Controllers
{
    ...
    public class DocumentTypeController : System.Web.Mvc.Controller
    {
        ...
        {
            var DocType = new DocumentTypeService(_unitOfWork).FindByName(DocumentTypeConstants.DocumentType.DocumentTypeName);
            int DocTypeId = 0;

            if (DocType != null)
                DocTypeId = DocType.DocumentTypeId;
            else
                return View("~/Views/Shared/InValidSettings.cshtml").Warning("Document Type named " + DocumentTypeConstants.DocumentType.DocumentTypeName + " is not defined in database.");
        ...
        public ActionResult Post(DocumentType pt)
        {
            if (ModelState.IsValid)
            {
                if (pt.DocumentTypeId <= 0)
                {
                    ...
                    {
                        DocTypeId = new DocumentTypeService(_unitOfWork).FindByName(DocumentTypeConstants.DocumentType.DocumentTypeName).DocumentTypeId,
        ...
        public ActionResult Report()
        {
            ...
            Dt = new DocumentTypeService(_unitOfWork).FindByName(DocumentTypeConstants.DocumentType.DocumentTypeName);
