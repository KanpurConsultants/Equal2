CREATE PROCEDURE Web.spMaterialPlanLineNewController_GetProdOrders_ListProdOrderBalanceForMPlan 
...
SELECT @PlanDocTypeId=H.DocTypeId, @DivisionId=H.DivisionId,@BuyerId=H.BuyerId, @SiteId= H.SiteId  
...
LEFT JOIN Web.ProdOrderHeaders POH ON VProdOrder.ProdOrderHeaderId = POH.ProdOrderHeaderId
...
AND ( @BuyerId is null or POH.BuyerId IN (SELECT Items FROM [web].[Split] (@BuyerId, ',')))
